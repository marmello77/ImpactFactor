...
jcreco <- read.table("data/jcr ecology 2024.csv", sep=",", 
...
jcreco.med <- summarise(jcreco, 
                        IFmedian = median(X2024.JIF, na.rm = T))
jcreco.med
...
p1 <- ggplot(jcreco, aes(x=X2024.JIF)) +
  ...
  xlab("Impact factor 2024: Ecology") + 
  ylab("Frequency") +
...
p1
png(filename= "figures/ecology 2024.png", 
    res= 300,  height= 16, 
    width=26, unit="cm")
p1
dev.off()
...
jcroce <- read.table("data/jcr oceanography 2024.csv", sep=",", 
                     skip=2, fill=T, header=T, row.names=NULL,
                     na.strings = "N/A")
...
p2 <- ggplot(jcrbio, aes(x=X2024.JIF, fill=Category)) +
  geom_density(alpha=.3, position="identity", color = F) +
  xlab("Impact factor 2024: Biodiversidade Capes") + 
  ylab("Frequency") +
...
p2
png(filename= "figures/biodiversity 2024.png", 
    res= 300,  height= 16,
    width=26, unit="cm")
p2
dev.off()
