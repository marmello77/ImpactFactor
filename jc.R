...
jcreco = read.delim("ecology.txt", na = "Not Available")
is.na(jcreco)
dim(jcreco)
colnames(jcreco)
head(jcreco)
class(jcreco$Journal.Impact.Factor)

#Summarise
jcreco.med <- summarise(jcreco, IFmedian = median(Journal.Impact.Factor, na.rm = T))
jcreco.med
eco.med = round(jcreco.med[1,1], digits = 2)
str(eco.med)

jcreco.qua1 <- summarise(jcreco, IFqua1 = quantile(Journal.Impact.Factor, na.rm = T))
class(jcreco.qua1)
str(jcreco.qua1)
eco.qua1 = round(jcreco.qua1[2,1], digits = 2)
str(eco.qua1)

jcreco.qua2 <- summarise(jcreco, IFqua2 = quantile(Journal.Impact.Factor, na.rm = T))
class(jcreco.qua2)
str(jcreco.qua2)
...
p1 = ggplot(jcreco, aes(x=Journal.Impact.Factor)) +
  ...
  geom_text(aes(eco.med, 26, label = eco.med, angle = 0, hjust = -0.1, vjust = 0), size = 5, color = "blue") +
  geom_vline(data=jcreco.qua1, aes(xintercept=eco.qua1),
             linetype="dashed", size=0.5, color = "blue") +
  geom_text(aes(eco.qua1, 26, label = eco.qua1, angle = 0, hjust = -0.1, vjust = 0), size = 3, color = "blue") +
  geom_vline(data=jcreco.qua1, aes(xintercept=eco.qua2),
             linetype="dashed", size=0.5, color = "blue") +
  geom_text(aes(eco.qua2, 26, label = eco.qua2, angle = 0, hjust = -0.1, vjust = 0), size = 3, color = "blue")
...
#Create and inspect object
jcrbio = read.delim("biodiversity.txt", na = "Not Available")
is.na(jcrbio)
dim(jcrbio)
colnames(jcrbio)
head(jcrbio)
...
#Summarise
jcrbio.med <- ddply(jcrbio, "Category", summarise, IFmedian = median(Journal.Impact.Factor, na.rm = T))
jcrbio.med
...
p2 = ggplot(jcrbio, aes(x=Journal.Impact.Factor, fill=Category)) +
  ...
  geom_text(aes(eco.med, 0.60, label = eco.med, angle = 0, hjust = 0, vjust = 0), size = 3) +
  geom_text(aes(oce.med, 0.62, label = oce.med, angle = 0, hjust = 0, vjust = 0), size = 3) +
  geom_text(aes(pla.med, 0.64, label = pla.med, angle = 0, hjust = 0, vjust = 0), size = 3) +
  geom_text(aes(zoo.med, 0.66, label = zoo.med, angle = 0, hjust = 0, vjust = 0), size = 3)
